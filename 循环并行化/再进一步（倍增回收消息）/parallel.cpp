...
int main(){
	...
	if (my_rank == 0) {
		...
		while (l <= r) sum += l, l++;
		for (int i = 1; i + my_rank < comm_sz; i += i) {
			MPI_Recv(&v, 1, MPI_LONG_LONG, i + my_rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			sum += v;
		}
		printf("%lld\n",sum);
	}else{
		...
		for (int i = 1;; i += i) {
			if ( my_rank & i ){
				MPI_Send(&sum, 1, MPI_LONG_LONG, my_rank - i, 1, MPI_COMM_WORLD);
				break;
			}
			if (my_rank + i < comm_sz){
				MPI_Recv(&v, 1, MPI_LONG_LONG, my_rank + i, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				sum += v;
			}
		}
	}
